  ...
  public class DoWork extends AsyncTask<Void, IOException, Boolean> {
      ...
      while (!isCancelled()) {
        ...
        try {
          ...
          if (PasswordRequiredException.class.isAssignableFrom(e.getClass())
            ...
            BaseApplication.toast(extractService, extractService.getString(R.string.error));
            paused = true;
            publishProgress(e);
          }
        }
    ...
    protected void onProgressUpdate(IOException... values) {
      ...
      GeneralDialogCreation.showPasswordDialog(
          BaseApplication.getInstance().getMainActivityContext(),
          (MainActivity) BaseApplication.getInstance().getMainActivityContext(),
          BaseApplication.getInstance().getUtilsProvider().getAppTheme(),
    ...
    public void onPostExecute(Boolean hasInvalidEntries) {
      ...
      if (!hasInvalidEntries)
        BaseApplication.toast(extractService, getString(R.string.multiple_invalid_archive_entries));
    }
