...
public class CryptHandler {
  ...
  private static class CryptHandlerHolder {
    private static final CryptHandler INSTANCE =
        new CryptHandler(BaseApplication.getInstance().getExplorerDatabase());
  }
  ...
  public void addEntry(EncryptedEntry encryptedEntry) {
    BaseApplication.runInBackground(() -> database.encryptedEntryDao().insert(encryptedEntry));
  }

  public void clear(String path) {
    BaseApplication.runInBackground(() -> database.encryptedEntryDao().delete(path));
  }

  public void updateEntry(EncryptedEntry oldEncryptedEntry, EncryptedEntry newEncryptedEntry) {
    BaseApplication.runInBackground(() -> database.encryptedEntryDao().update(newEncryptedEntry));
  }
