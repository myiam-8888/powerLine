...
public class VlcVideoPlayer implements MediaPlayerControl, Handler.Callback, IVLCVout.Callback {
    ...
    public boolean handleMessage(Message msg) {
        synchronized (VlcVideoPlayer.class) {
            switch (msg.what) {
                case MSG_START:
                    Log.i(TAG, "-----HandlerThread init 1=" + isInitStart);
                    if (isInitStart) {
                        opendVideo();
                    }
                    Log.i(TAG, "-----HandlerThread init 2=" + isInitStart);
                    break;
                case MSG_STOP:
                    Log.i(TAG, "-----HandlerThread stop 3=" + isInitStart);
                    release();
                    Log.i(TAG, "-----HandlerThread stop 4=" + isInitStart);
                    break;
            }
        }
    ...
    private void opendVideo() {
        ...
        if (!mMediaPlayer.getVLCVout().areViewsAttached() && isAttached && surface != null) {
            ...
            Log.i(tag, "setVideoSurface   attachViews");
        }
        ...
        isInitPlay = true;
        othereMedia = false;
        Log.i(tag, "isAttached=" + isAttached + " isInitStart=" + isInitStart);
    ...
    private void reStartPlay() {
        if (isAttached && isLoop && isPrepare()) {
            Log.i(tag, "reStartPlay setMedia");
    ...
    private void release() {
        ...
        if (mMediaPlayer != null && isInitPlay) {
            Log.i(tag, "release SaveState  isAttachSurface=" + isAttachSurface);
            isInitPlay = false;
            ...
            if (isSaveState) {
                ...
                if (media != null) {
                    ...
                    mMediaPlayer.stop();
                    Log.i(tag, "release setMedia null");
    ...
    private void onEventNative(final MediaPlayer.Event event) {
        switch (event.type) {
            case MediaPlayer.Event.Stopped:
                ...
                canSeek = false;
                canPause = false;
                Log.i(tag, "Stopped  isLoop=" + isLoop + "  ");
                reStartPlay();
                break;
            case MediaPlayer.Event.EndReached:
                if(null!=mVlcVideoPlayerOnComplete){
                    mVlcVideoPlayerOnComplete.onComplete();
                }
            ...
            case MediaPlayer.Event.Playing:
                ...
                if (!isAttached || !mMediaPlayer.getVLCVout().areViewsAttached()) {//禁止双线程后台运行
                    Log.i(tag, "---多线程出错----没有surface  禁止双线程后台运行");
                    //  mMediaPlayer.stop();
                    if (mMediaPlayer != null) {
                        mMediaPlayer.pause();
                    }
            ...
            case MediaPlayer.Event.SeekableChanged:
                ...
                Log.i(tag, "SeekableChanged=" + canSeek);
                break;
            case MediaPlayer.Event.PausableChanged:
                canPause = event.getPausable();
                Log.i(tag, "PausableChanged=" + canPause);
                break;
            ...
            case MediaPlayer.Event.MediaChanged:
                Log.i(tag, "MediaChanged=" + event.getEsChangedType());
                break;
            default:
    ...
    public interface vlcVideoPlayerOnComplete{
        void onComplete();
    }

    private vlcVideoPlayerOnComplete mVlcVideoPlayerOnComplete;


    public void setVlcVideoPlayerOnComplete(vlcVideoPlayerOnComplete mVlcVideoPlayerOnComplete){
        this.mVlcVideoPlayerOnComplete=mVlcVideoPlayerOnComplete;
    }
